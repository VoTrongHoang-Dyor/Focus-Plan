...
- **Feature mới / thay đổi kiến trúc đáng kể**:
  ...
  2. Nếu Decision Log chưa đủ rõ (còn câu hỏi mở/thiết kế chưa chốt) → dùng skill `superpowers:brainstorming` để làm rõ ý tưởng/thiết kế. Một khi đã trigger, không bỏ qua vì "thấy đơn giản". Nếu Decision Log đã đủ rõ → bỏ qua bước này, tránh hỏi lại user điều librarian vừa hỏi xoáy.
...
- **`superpowers:brainstorming`** — Feature mới / thay đổi kiến trúc đáng kể, chỉ khi Decision Log từ librarian (hoặc ý tưởng user nêu, nếu chưa cần librarian) chưa đủ rõ — còn câu hỏi mở/thiết kế chưa chốt. Nếu Decision Log đã đủ rõ thì bỏ qua bước này. Một khi đã trigger thì bắt buộc chạy, không bỏ qua vì "thấy đơn giản".
