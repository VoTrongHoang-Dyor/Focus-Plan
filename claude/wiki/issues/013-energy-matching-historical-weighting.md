...
Nâng cấp energy-matching từ rule tĩnh (issue 003) sang có trọng số theo lịch sử thực tế: dùng Pomodoro completion rate theo khung giờ của từng user để tự điều chỉnh trọng số ưu tiên slot. Vẫn deterministic/thống kê, không phải LLM reasoning.

## Acceptance criteria

- [ ] Engine đọc được completion rate theo khung giờ từ dữ liệu Pomodoro đã lưu (issue 006).
...
- [ ] Có thể so sánh A/B được giữa rule tĩnh (003) và rule có trọng số (011) trên cùng bộ dữ liệu test.
